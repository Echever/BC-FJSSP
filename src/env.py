...
class FJSSPEnv(gym.Env):
    ...
    def generate_instance(self, instance):
        ...
        self.data['job', 'change_2', 'machine'].edge_index = torch.LongTensor(aux_list2).T
        ...
        for i in range(len(self.operations)):
            ...
            for j in range(len(o)):
                ...
                if t!=0:
                    ...
                    aux_list_features.append([t, t/np.sum(o), t/self.all_pendings[i], 0])
        ...
        for i in range(self.num_jobs):
            ...
            for j in self.jobs[i]:
                ...
                self.data["operation"].x[j, 2] = np.mean(aux[np.array(aux)!=0])
                self.data["operation"].x[j, 3] = np.min(aux[np.array(aux)!=0])
                self.data["operation"].x[j, 4] = np.sum(aux[np.array(aux)!=0])/self.all_pendings[j]
                o_index+=1
        ...
        for j_id in range(len(self.jobs)):
            ...
            for m in range(len(oper)):
                ...
                if t!=0:
                    ...
                    aux_list_features.append([t, t/np.sum(oper), t/np.sum(oper), 0])
    ...
    def calculate_next_state(self):
        ...
        for m in range(len(self.state["machine"].x)):
            ...
            if mask.any().item():
                ...
                self.state['job', 'change_2', 'machine'].edge_index = self.state['job', 'change_2', 'machine'].edge_index[:, self.state['job', 'change_2', 'machine'].edge_index[1,:] != m]
            ...
            if mask.any().item():
                ...
                for index, p in enumerate(self.state["machine", "exec", "job"].edge_index[:, mask].T): 
                    if self.operations_ends[p[1]] <  self.state["machine"].x[m, 0]:
                        self.state["machine", "exec", "job"].edge_attr[index, 0] += self.state["machine"].x[m, 0] - self.operations_ends[p[1]] 
    ...
    def step(self, action):
        ...
        if (int(self.current_operations[sel_job])) == self.jobs[sel_job][-1]:
            ...
            self.state['job', 'listens', 'job'].edge_index = self.state['job', 'listens', 'job'].edge_index[:, self.state['job', 'listens', 'job'].edge_index[1,:] != sel_job]
            self.state['job', 'change_2', 'machine'].edge_index = self.state['job', 'change_2', 'machine'].edge_index[:, self.state['job', 'change_2', 'machine'].edge_index[0,:] != sel_job]
    ...
    def sample(self):
        return random.choice([i for i in range(len(self.state['machine', 'exec', 'job']))])
